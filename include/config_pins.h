...
constexpr uint8_t PIN_DOOR_SENSOR = 13;

// DS18B20 (OneWire) temperature sensor
constexpr uint8_t PIN_TEMP_SENSOR = 4;

// Output pins (active-LOW relays: LOW=ON, HIGH=OFF)
constexpr uint8_t PIN_HEATER_RELAY = 16;    // Relay CH1
constexpr uint8_t PIN_MOTOR_FWD_RELAY = 17; // Relay CH2
constexpr uint8_t PIN_MOTOR_REV_RELAY = 23; // Relay CH3
constexpr uint8_t PIN_AUX_RELAY = 15;       // Relay CH4 (reserved / future use)

// Audible feedback / alarm
constexpr uint8_t PIN_BUZZER = 5; // tone() output
...
constexpr uint8_t PIN_KEYPAD_ROW[4] = {14, 27, 26, 25};
constexpr uint8_t PIN_KEYPAD_COL[4] = {33, 32, 18, 19};
